...
import { DEFAULT_MEMBER_NAME, LocalizedError, PROOFSPACE_STRATEGY, isAuthroizationRequired } from '@smartapps-poll/common'
...
export const ProofspaceAuthorizationChoice: FunctionComponent<ProofspaceAuthorizationChoiceProps> = ({
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        let screen = 'choice'
        if (ctx.config.hideProofspace) {
          screen = 'proofspace-info'
    ...
    <Navigator navigation={nav}>
      <ResultBox status={status} error={error} retry={retry} fullWidth onSuccess={
        ...
        <Screen screen="choice">
          <Grid item container xs={12} minWidth={250} minHeight={600} p={1} direction="column" justifyContent="center" alignItems="center">
            ...
            <Grid item xs={1} minWidth={200}>
              {strategy != null
                ? <ButtonStackStrategy strategy={strategy as string} navigate={navigate} poll={poll as PollInfo} authorize={authorizeInstantly} />
                : undefined}
            </Grid>
        ...
        <Screen screen="authenticate">
          <Grid item container xs={12} minWidth={250} p={1} direction="column" justifyContent="center" alignItems="stretch">
            {poll == null
              ? undefined
              : <ProofspaceAuthorization poll={poll} onSuccess={onAuthSuccess} onFailure={onAuthFailure} />}
          </Grid>
        </Screen>
        <Screen screen="proofspace-info">
...
export interface ProofspaceAuthorizationChoiceProps extends ProofspaceAuthChoiceAsyncProps {
  onBack: () => void
  pollId: string
}
